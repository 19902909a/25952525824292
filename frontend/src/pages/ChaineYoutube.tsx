...
import { Youtube, ExternalLink, Calendar, Sparkles, Volume2, VolumeX, ArrowRight, Play, Search, X } from "lucide-react";
...
const ChaineYoutube = () => {
  ...
  const filteredList = useMemo(() => {
    ...
    return list.filter((v) => {
      const haystack = `${v.title || ""} ${v.episode || ""}`.toLowerCase();
  ...
  return (
    ...
    <section className="container mx-auto px-4 lg:px-8 py-16">
      ...
      <div className="mt-6 flex flex-wrap items-center gap-3" data-testid="youtube-search-toolbar">
        <div className="relative flex-1 min-w-[260px] max-w-xl">
          <Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
          <Input
            value={query}
            onChange={(event) => setQuery(event.target.value)}
            placeholder="Rechercher par titre, série ou épisode…"
          ...
          {query && (
            <button
              ...
              data-testid="youtube-search-clear"
            >
              <X className="h-3.5 w-3.5" />
            </button>
          )}
        </div>
        <div className="text-xs text-muted-foreground" data-testid="youtube-search-count">
          {filteredList.length} vidéo{filteredList.length > 1 ? "s" : ""}
          {query && list.length !== filteredList.length ? ` sur ${list.length}` : ""}
        </div>
      </div>
    </section>
      ...
      <section className="container mx-auto px-4 lg:px-8 pb-16 grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
        {filteredList.length === 0 ? (
          <div className="col-span-full text-center text-sm text-muted-foreground py-10" data-testid="youtube-search-empty">
